...
public class PerfectStrategy extends WinningHands {
	...
	public PerfectStrategy(Hand hand) {
		super(hand);
		eqRank = -1;
		eqRank2 = -1;
		flushSuit = -1;
	...
	public int[] decision(Hand hand) {
		...
		else if(resultStrat == 6 || resultStrat == 14 || resultStrat == 20 || resultStrat == 27)
		{
			...
			{
				if(matHand[flushSuit][i] != 0)
				{
					holdCards[matHand[flushSuit][i] - 1] = 1;
				}
				if(i == 0 && matHand[flushSuit][12] != 0)
				{
					holdCards[matHand[flushSuit][12] - 1] = 1;
				}
			}
		}
		...
		for(int i = 0; i < 4; i++)
		{
			for(int j = -1; j < 9; j++)
			{
				...
				if(j == -1)
				{
					...
					if(matHand[i][12] != 0)
						straight_flush++;
					straight_flush += super.sumRow(matHand[i], 0, 4);
				...
				{
					aceException = 0;
					straight_flush += super.sumRow(matHand[i], j, 5);
				...
				if(straight_flush == 3)
				{
					...
					else if(((nGaps == 1) || (nGaps == 2 && nHC == 1) || (j == -1 && matHand[i][12] != 0) || (matHand[i][0] != 0 && matHand[i][1] != 0 &&  matHand[i][2] != 0)) && resultStrat > 20)
					{
						straightBeg = j + aceException;
						flushSuit = i;
						resultStrat = 20;
	...
	public void isFlush() {
		...
		for(int i = 0; i < 4; i++)
		{
			RF = super.sumRow(super.matHand[i], 8, 5);
			F = super.sumRow(super.matHand[i], 0, 13);
	...
	{
		return super.sumRow(super.matHand[row], 9, 4);
	}
