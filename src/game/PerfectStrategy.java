...
/**
 * Performs the perfect playing strategy by finding the type of hand the player carries and holding specific cards according to that type of hand
...
public class PerfectStrategy {
	...
	/**
	 * Constructor of PerfectStrategy, which initializes the matrix and the variables
	 * @param _hand Linked List with the cards the player has at the moment
	 */
	...
	/**
	 * Method that, according to which type of hand the player has, decides which cards to hold and which cards to discard
	 * @return int[] Array with the cards the player is to hold
	 */
	public int[] decision() {
	...
	/**
	 * Finds if the current hand corresponds to any type of Straight Hand
	 * Includes types: 1, 4, 6, 11, 14, 15, 19, 20, 21, 27, 32
	 */
		...
		for(int i = 0; i < 4; i++)
		{
			for(int j = -1; j < 9; j++)
			{
				...
				if(straight == 4)
				{
					...
					if(aceException != 0)
						ace = 12;
					if((((doubleBonus.sumCol(ace) != 0 && (doubleBonus.sumCol(j  + aceException + 4)) == 0) || (doubleBonus.sumCol(ace) == 0  && (doubleBonus.sumCol(j  + aceException + 4)) != 0)) && j + aceException + 4 != 12 && nGaps == 0) && aceException == 0)
					{
						if(resultStrat > 11)
						{
							straightBeg = j + aceException;
	...
	/**
	 * Finds if the current hand corresponds to any type of Flush Hand
	 * Includes types: 1, 2, 4, 9, 10, 17, 25, 33
	 */
	public void isFlush() {
		int RF = 0, F = 0, n_HC = 0;
		for(int i = 0; i < 4; i++)
	...
	/**
	 * Finds if the current hand corresponds to any type of Hand with cards of the same rank
	 * Includes types: 3, 4, 5, 7, 8, 12
	 */
	...
	/**
	 * Finds if the current hand has any two suited high cards (including the Ten)
	 * Includes types: 16, 18, 23, 26, 30
	 */
	...
	/**
	 * Finds if the current hand corresponds to an hand with unsuited cards 
	 * Includes types: 13, 22, 24, 28, 29, 31
	 */
	...
	/**
	 * Counts how many high cards of the same suit are there in the player's hand
	 * @param row Suit for which to count the number of high cards
	 * @return int Number of high cards in hand
	 */
	...
	/**
	 * Checks if there's an High Card of specific rank
	 * @param col Rank number in where to check
	 * @return boolean Existence of the High Card in the player's hand
	 */
	...
	/**
	 * Counts how many gaps exist between certain ranks in a specific suit
	 * @param row Suite in which to count number of gaps
	 * @param col Rank in which to begin counting the number of gaps
	 * @return int Number of gaps found
	 */
	...
	/**
	 * Counts how many gaps exist between certain ranks taking into account all suits
	 * @param col Rank in which to begin counting the number of gaps
	 * @return int Number of gaps found
	 */
